...
_CONFIG2(PLL_96MHZ_ON & IESO_OFF & FCKSM_CSDCMD & OSCIOFNC_OFF & POSCMOD_HS & FNOSC_PRIPLL & PLLDIV_NODIV & IOL1WAY_OFF & DISUVREG_OFF)
...
#define ProgramMemStart					0x00001400 //Beginning of application program memory (not occupied by bootloader).  **THIS VALUE MUST BE ALIGNED WITH BLOCK BOUNDRY** Also, in order to work correctly, make sure the StartPageToErase is set to erase this section.
	...
	#define MaxPageToEraseNoConfigs		168		 //Last full page of flash on the PIC24FJ256GB110, which does not contain the flash configuration words. (or our custom serial number /calibration)
	...
	#define ProgramMemStopNoConfigs		0x0002A200 //Must be instruction word aligned address.  This address does not get updated, but the one just below it does: 
	...
	#define	BeginPageToErase			5		 //Bootloader and vectors occupy first six 1024 word (1536 bytes due to 25% unimplemented bytes) pages
	#define MaxPageToEraseNoConfigs		128		 //Last full page of flash on the PIC24FJ192GB110, which does not contain the flash configuration words. (or our custom serial number /calibration)
	...
	#define ProgramMemStopNoConfigs		0x00020200 //Must be instruction word aligned address.  This address does not get updated, but the one just below it does: 
                                                        //IE: If AddressToStopPopulating = 0x200, 0x1FF is the last programmed address (0x200 not programmed)	
	...
	#define	BeginPageToErase			5		 //Bootloader and vectors occupy first six 1024 word (1536 bytes due to 25% unimplemented bytes) pages
	#define MaxPageToEraseNoConfigs		83		 //Last full page of flash on the PIC24FJ128GB110, which does not contain the flash configuration words. (or our custom serial number /calibration)
	...
	#define ProgramMemStopNoConfigs		0x00014E00 //Must be instruction word aligned address.  This address does not get updated, but the one just below it does: 
                                                        //IE: If AddressToStopPopulating = 0x200, 0x1FF is the last programmed address (0x200 not programmed)	
	...
	#define	BeginPageToErase			5		 //Bootloader and vectors occupy first six 1024 word (1536 bytes due to 25% unimplemented bytes) pages
	#define MaxPageToEraseNoConfigs		40		 //Last full page of flash on the PIC24FJ64GB110, which does not contain the flash configuration words. (or our custom serial number /calibration)
	...
	#define ProgramMemStopNoConfigs		0x0000A200 //Must be instruction word aligned address.  This address does not get updated, but the one just below it does: 
                                                        //IE: If AddressToStopPopulating = 0x200, 0x1FF is the last programmed address (0x200 not programmed)	
